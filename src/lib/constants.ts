import { Question, DiagnosticResult, CareerType, DynamicCTA, SommelierRecommendation, HiddenList, SommelierAnalysis } from '@/types';
...
export const CAREER_TYPES_DATA: Record<CareerType, Omit<DiagnosticResult, 'radarChart' | 'type'>> = {
    AI_MASTER: {
        ...
        actionPlan: [
            '【AIを飼いならす】日常のメールや資料作成をAIに任せ、自分を「1日1時間」解放する。AIが自分の意図を汲み取る驚きを体感する。',
            '【自動化ツールでブースト】複数のAIを連携させる専門ツールを導入。自分が寝ている間もAIがリサーチや分析を完遂する「自動収益の土台」を構築する。',
            '【AI時代の支配者】ツールを使いこなし、人の10倍の速さでアウトプット。周囲が驚くスピードで新しいビジネスを次々と立ち上げる。',
        ],
    },
    DIGITAL_ARTISAN: {
        ...
        actionPlan: [
            '【価値の再発見】AIには描けない自分の「こだわり」を抽出し、デジタル化する。自分の感性が世界に通用する確信を得る。',
            '【表現のアップデート】自身の技術を拡張するクリエイティブツールを導入。手作業の良さを残しつつ、デジタルならではの「量産と発信」を両立させる。',
            '【ハイブリッド職人】「唯一無二の技術 × 最先端の届け方」を確立。ファンが列をなす、時代に媚びない独自の経済圏を築く。',
        ],
    },
    COMMUNITY_WEAVER: {
        ...
        actionPlan: [
            '【繋がりの可視化】バラバラだった人間関係を整理し、小さなオンラインの場を作る。自分の言葉で人が動く楽しさを再確認する。',
            '【集客・管理のシステム化】コミュニティ運営を自動化するツールを導入。事務作業をゼロにし、一人ひとりと深く対話する「熱狂の醸成」に没頭する。',
            '【プラットフォームの主】あなたが介在するだけで価値が生まれる「仕組みとしてのコミュニティ」を完成させ、圧倒的な人脈資産を手にする。',
        ],
    },
    DATA_ALCHEMIST: {
        ...
        actionPlan: [
            '【隠れた法則の発見】身近な数字を整理し、「なぜ売れたか」「なぜ失敗したか」の根拠を突き止める。直感が確信に変わる快感を知る。',
            '【予測モデルの導入】高度な解析ソフトを実戦投入。勘に頼らず「未来の売上」を高い精度で予測し、外さない経営判断の武器を手に入れる。',
            '【利益の予言者】データから「次に打つべき一手」を常に提示し続け、組織やクライアントに莫大な利益をもたらす中核存在になる。',
        ],
    },
    INNOVATION_ARCHITECT: {
        ...
        actionPlan: [
            '【常識の破壊】既存のビジネスモデルの弱点を見抜き、改善のプロトタイプ（試作）を作る。自分のアイデアが形になる手応えを掴む。',
            '【高速実装ツールの活用】アイデアを即座に形にする開発不要のツールを導入。朝思いついた企画を夕方にはリリースする「超速PDCA」を実現する。',
            '【市場の開拓者】誰も見たことがないサービスを世に送り出し、競合が気づく前に市場のシェアを独占する圧倒的な先行優位を築く。',
        ],
    },
    ETHICAL_GUARDIAN: {
        ...
        actionPlan: [
            '【リスクに気づく】便利さの裏にあるデータの扱いや、AIがつく「嘘」を見抜く正しい知識を身につける。自分の身を自分で守る術を学ぶ。',
            '【ルールを整える】自分のチームで、誰もが安心して技術を使えるための運用ルールを提案する。周囲から「情報の番人」として信頼を得る。',
            '【信頼の象徴】技術の進化と人間らしさのバランスを保つアドバイザーとして、確固たる地位を築き、高度な意思決定に加わる。',
        ],
    },
    GLOBAL_NOMAD: {
        ...
        actionPlan: [
            '【どこでも仕事の準備】場所を選ばず成果を出せる環境と、文章だけで正確に伝える技術を磨く。「どこでも生きていける」自信の芽が育つ。',
            '【グローバル連携ツールの導入】AI翻訳や国際決済ツールを使いこなし、日本にいながら「ドルの収入」を現実にする最初の一歩を踏み出す。',
            '【完全な場所の自由】特定のオフィスに縛られず、好きな場所から価値を提供し続ける基盤を作り、世界を舞台に働くライフスタイルを確立する。',
        ],
    },
    REALITY_BRIDGER: {
        ...
        actionPlan: [
            '【別世界の体験】最新の仮想空間を実際に歩き回り、そこで人々が何を求めているのかを肌で感じる。「新しい市場」の熱量を体感する。',
            '【直感的制作ツールの習得】難しい言語を使わず空間を創れるツールを導入し、自分のアイデアを三次元で具現化して周囲を驚かせる。',
            '【次世代の空間プロデューサー】現実とデジタルを融合させた、新しい顧客体験やイベントを自らプロデュースし、独自の経済圏を創出する。',
        ],
    },
    SUSTAINABLE_STRATEGIST: {
        title: '地球を救う「永続の戦略家」',
        ...
        actionPlan: [
            '【無駄の特定】今の業務の中で、自分や周囲を消耗させている要因を特定し、排除する。持続可能な働き方のプロトタイプを自分自身で試す。',
            '【効率化・最適化ツールの導入】無駄を自動で検知し、コストを下げながら社会貢献できる仕組みをシステムとして構築し、余裕を生み出す。',
            '【未来の価値の証明】10年後も通用する持続可能なモデルとして成果を出し、長期的な視点を持つリーダーとして周囲から認められる。',
        ],
    },
    LEGACY_KEEPER: {
        ...
        actionPlan: [
            '【知恵の言語化】ブラックボックス化した古い仕組みを整理し、誰でもわかる「地図」として書き出す。自分の価値を「属人性」から解放する。',
            '【ドキュメント・移行AIの活用】AIを使ってブラックボックスを自動解析し、最新環境へ乗り換えるための設計図を作り、変革のリーダーシップを握る。',
            '【新旧融合のスペシャリスト】過去の価値を最新技術で甦らせ、組織に「揺るぎない安定」と「進化の基盤」をもたらす唯一無二の功労者になる。',
        ],
    },
    GIG_SURVIVOR: {
        ...
        actionPlan: [
            '【看板の磨き上げ】自分の強みをパッケージ化し、高単価なメニューを提示する。安売りから脱却し、自分の価値が正当に評価される喜びを得る。',
            '【営業・事務の自動化】案件獲得から契約までを代行するプラットフォームを導入。営業をゼロにし、専門スキルの提供だけで稼ぐ体制を作る。',
            '【完全自立型プロ】自由な時間と高い報酬を両立。場所にも組織にも縛られず、自分の名前だけで生き抜く真の自由を手にする。',
        ],
    },
    ENDANGERED_WORKER: {
        ...
        actionPlan: [
            '【生存スキルの習得】毎日15分AIに触れる習慣を作り、簡単な業務をAIに代行させる。得体の知れない恐怖が「味方に変える期待」に変わる。',
            '【キャリア進化講座の受講】AI時代に生き残るための専門講座やスキル習得ツールを開始。自分の市場価値が具体的に上がっていく実感を得る。',
            '【逆転のサバイバー】AIに仕事を奪われる側から、AIを使って価値を生む側へ。変化を味方につけ、次のキャリアを自分で選べる強さを手にする。',
        ],
    },
};
